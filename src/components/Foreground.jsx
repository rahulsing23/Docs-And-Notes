...
const Foreground = () => {
  ...
  const handleSearchedQuery = async () =>{
    ...
    try {
      const q = query(collection(db, "workspaceDocument"),
      or( where('title', '==', inputquery.toLowerCase()), where('priority', '==', inputquery.toLowerCase()))
    
      );
      const querySnapshot = await getDocs(q);
      const Output = querySnapshot.docs.filter((doc)=> {
      ...
      console.log("Error in handleSearchQuery function of Foreground :: ",error.message)
    }
